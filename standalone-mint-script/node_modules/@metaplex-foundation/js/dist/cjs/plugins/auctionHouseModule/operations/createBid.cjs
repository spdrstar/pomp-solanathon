...
const createBidBuilder = async (metaplex, params, options = {}) => {
  ...
  const tokenAccount = (_params$tokenAccount = params.tokenAccount) !== null && _params$tokenAccount !== void 0 ? _params$tokenAccount : params.seller ? metaplex.tokens().pdas().associatedTokenAccount({
    mint: params.mintAccount,
    owner: params.seller,
    programs
  }) : null;
  ...
  let buyInstruction = tokenAccount ? mplAuctionHouse.createBuyInstruction({ ...accounts,
    tokenAccount
  ...
  const buySigners = [buyer, authority, params.auctioneerAuthority].filter(Signer.isSigner); // Update the accounts to be signers since it's not covered properly by MPL due to its dynamic nature.
