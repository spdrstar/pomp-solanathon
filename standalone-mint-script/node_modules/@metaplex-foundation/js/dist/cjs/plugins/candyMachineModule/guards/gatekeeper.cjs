...
const gatekeeperGuardManifest = {
  name: 'gatekeeper',
  settingsBytes: 33,
  settingsSerializer: Serializer.mapSerializer(Serializer.createSerializerFromBeet(mplCandyGuard.gatekeeperBeet), settings => ({ ...settings,
  ...
  mintSettingsParser: ({
    ...
    var _mintSettings$tokenAc;

    const gatewayProgram = metaplex.programs().getGateway(programs);
    const tokenAccount = (_mintSettings$tokenAc = mintSettings === null || mintSettings === void 0 ? void 0 : mintSettings.tokenAccount) !== null && _mintSettings$tokenAc !== void 0 ? _mintSettings$tokenAc : Pda.Pda.find(gatewayProgram.address, [payer.publicKey.toBuffer(), buffer.Buffer.from('gateway'), buffer.Buffer.from([0, 0, 0, 0, 0, 0, 0, 0]), settings.network.toBuffer()]);
