...
const deserializeCandyMachineHiddenSection = (buffer, itemsAvailable, itemsRemaining, configLineSettings, offset = 0) => {
  ...
  const [itemsLoadedMap] = FeatureFlags.deserializeFeatureFlags(buffer, itemsAvailable, offset, false);
  const itemsLoadedMapSize = Math.floor(itemsAvailable / 8) + 1;
  offset += itemsLoadedMapSize; // Items left to mint for random order only.

  const itemsLeftToMint = beet__namespace.uniformFixedSizeArray(beet__namespace.u32, itemsAvailable).read(buffer, offset).slice(0, itemsRemaining); // Helper function to figure out if an item has been minted.
