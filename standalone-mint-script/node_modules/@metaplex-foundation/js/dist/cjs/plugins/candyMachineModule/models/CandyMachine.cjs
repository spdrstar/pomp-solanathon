...
const toCandyMachine = (account, candyGuard = null) => {
  ...
  return {
    ...
    isFullyLoaded,
    itemSettings,
    featureFlags: FeatureFlags.deserializeFeatureFlags(BigNumber.toBigNumber(parsedAccount.data.features).toArrayLike(buffer.Buffer, 'le', 8), 64)[0],
    candyGuard
  };
};
...
const getCandyMachineSize = data => {
  ...
  return Math.ceil(constants.CANDY_MACHINE_HIDDEN_SECTION + // Number of currently items inserted.
  4 + // Config line data.
