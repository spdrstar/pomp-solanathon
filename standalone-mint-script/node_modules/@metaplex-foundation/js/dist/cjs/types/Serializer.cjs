...
const createSerializerFromBeet = beetArg => ({
  ...
  deserialize: (buffer, offset) => {
    const fixedBeet = beet__namespace.isFixableBeet(beetArg) ? beetArg.toFixedFromData(buffer, offset !== null && offset !== void 0 ? offset : 0) : beetArg;
    const reader = new beet__namespace.BeetReader(buffer, offset !== null && offset !== void 0 ? offset : 0);
    const value = reader.read(fixedBeet);
    return [value, reader.offset];
  }
});
const createSerializerFromSolitaType = (solitaType, description) => ({
  description: description !== null && description !== void 0 ? description : solitaType.name,
  serialize: value => {
...
const serialize = (value, serializer) => {
  try {
    ...
    throw new SdkError.FailedToSerializeDataError(serializer.description, {
      cause: error
    });
  }
};
const deserialize = (value, serializer) => {
  try {
    ...
    throw new SdkError.FailedToDeserializeDataError(serializer.description, {
      cause: error
    });
  }
};
function deserializeAccount(account, serializer) {
  ...
  try {
    ...
    throw new SdkError.UnexpectedAccountError(account.publicKey, serializer.description, {
      cause: error
    });
  }
}
