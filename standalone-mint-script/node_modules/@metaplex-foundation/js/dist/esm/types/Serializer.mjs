...
const createSerializerFromBeet = beetArg => ({
  ...
  deserialize: (buffer, offset) => {
    const fixedBeet = beet.isFixableBeet(beetArg) ? beetArg.toFixedFromData(buffer, offset !== null && offset !== void 0 ? offset : 0) : beetArg;
    const reader = new beet.BeetReader(buffer, offset !== null && offset !== void 0 ? offset : 0);
...
function deserializeAccount(account, serializer) {
  ...
  try {
    ...
    throw new UnexpectedAccountError(account.publicKey, serializer.description, {
      cause: error
    });
  }
}
