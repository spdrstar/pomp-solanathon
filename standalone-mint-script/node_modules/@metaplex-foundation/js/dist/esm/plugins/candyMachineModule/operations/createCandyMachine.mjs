...
const createCandyMachineBuilder = async (metaplex, params, options = {}) => {
  var _params$creators, _params$itemSettings, _params$initializeCan;
  ...
  const creators = (_params$creators = params.creators) !== null && _params$creators !== void 0 ? _params$creators : [{
    address: toPublicKey(authority),
    share: 100
  }];
  const itemSettings = (_params$itemSettings = params.itemSettings) !== null && _params$itemSettings !== void 0 ? _params$itemSettings : {
    type: 'configLines',
    prefixName: '',
    nameLength: 32,
    prefixUri: '',
  ...
  if (!withoutCandyGuard) {
    ...
    const createCandyGuard = metaplex.candyMachines().builders().createCandyGuard({
      ...
      guards: (_params$guards = params.guards) !== null && _params$guards !== void 0 ? _params$guards : {},
  ...
  return builder.add(await metaplex.system().builders().createAccount({
    ...
    if (!isSigner(authority)) {
      throw new ExpectedSignerError('authority', 'PublicKey', {
        problemSuffix: 'In order to create a Candy Machine with an associated ' + 'Candy Guard you must provide the authority as a Signer.'
      });
    }
